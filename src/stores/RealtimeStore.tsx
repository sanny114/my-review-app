...
interface RealtimeStoreContextType {
  ...
  // Actions
  addProblem: (problem: (Omit<Problem, 'id' | 'createdAt' | 'updatedAt'> & { id?: string })) => Promise<void>
...
export const RealtimeStoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Actions実装
  const addProblem = async (problemData: (Omit<Problem, 'id' | 'createdAt' | 'updatedAt'> & { id?: string })) => {
